...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BBK)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    connect(ui->buttonOverviewSend, SIGNAL(clicked()), this, SIGNAL(sendCoinsClicked()));
    connect(ui->buttonOverviewReceive, SIGNAL(clicked()), this, SIGNAL(receiveCoinsClicked()));
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BBK")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeBitBlocksAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitBlocksAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBitBlocksAmount = strAnonymizeBitBlocksAmount.remove(strAnonymizeBitBlocksAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitBlocksAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBitBlocksAmount * COIN) nMaxToAnonymize = nAnonymizeBitBlocksAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBitBlocksAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBitBlocksAmount));
        strAnonymizeBitBlocksAmount = strAnonymizeBitBlocksAmount.remove(strAnonymizeBitBlocksAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitBlocksAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeBitBlocksAmount == 0) {
