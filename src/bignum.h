...
// Copyright (c) 2024 Updated for OpenSSL 3.0 compatibility
...
/** C++ wrapper for BIGNUM (OpenSSL bignum) - Updated for OpenSSL 3.0 */
class CBigNum
{
    // Friend declarations for operators that need access to bn
...
public:
    ...
    // Access to underlying BIGNUM pointer
    BIGNUM* getBN() { return bn; }
    const BIGNUM* getBN() const { return bn; }

    // For backward compatibility - allows &obj to get BIGNUM*
    ...
    */
    ...
    std::string ToString(int nBase=10) const
    {
        ...
        {
            if (!BN_div(dv.bn, rem.bn, tmp.bn, bnBase.bn, pctx))
    ...
    */
    bool isPrime(const int checks=BN_prime_checks) const {
        CAutoBN_CTX pctx;
        int ret = BN_check_prime(bn, pctx, nullptr);
        if(ret < 0){
